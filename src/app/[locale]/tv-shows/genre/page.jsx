...
const Genre = () => {
  ...
  const getAllGenres = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_IP_URL}/Tv/genres?lang=${locale}`
      );
  ...
  const getTvShowsByGenre = async (page) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_IP_URL}/Tv/genresByName?genreName=${encodedGenre}&page=${page}&lang=${locale}&count=10`
      );
